...
// Staff KPI Report
export const getStaffKPIReportData = asyncHandler(async (req, res, next) => {
    const filters = req.query;
    const report = await getStaffKPIReport(filters);

    res.status(200).json({
        success: true,
        message: "Staff KPI report retrieved successfully",
        data: report,
    });
});
