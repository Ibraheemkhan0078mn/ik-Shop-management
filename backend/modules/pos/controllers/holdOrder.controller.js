...
export const updateHoldOrder = asyncHandler(async (req, res, next) => {
    ...
    holdOrder.items          = normalizedItems;
    holdOrder.subtotal       = toNum(subtotal);
    holdOrder.discountAmount = toNum(discountAmount);
    holdOrder.totalAmount    = toNum(totalAmount);
    if (customerName !== undefined) holdOrder.customerName = customerName;
    if (waiter       !== undefined) holdOrder.waiter       = waiter;
    if (note         !== undefined) holdOrder.note         = note;

    await holdOrder.save();

    res.status(200).json({ success: true, message: "Held order updated successfully", data: holdOrder });
});
