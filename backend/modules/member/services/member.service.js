import { create, find, findOne, findById, update, deleteOne, count } from "./member.crud.js";
...
const memberCreate = async (data) => {
    ...
    const createdMember = await create(data);
...
const memberUpdate = async (id, data) => {
    ...
    const existingMember = await findById(id);
    if (!existingMember) {
        throw new Error("Member not found");
    }

    const updatedMember = await update(id, data);
...
const getAllMemberData = async () => {
    return await find().sort({ createdAt: -1 }).lean();
};
...
const checkDuplicateMemberInstituteId = async (instituteId) => {
    return await findOne({ instituteId });
};
