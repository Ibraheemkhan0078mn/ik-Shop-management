                            ...
                            <tbody>
                                {returns.map(r => (
                                    <ReturnRow
                                        key={r._id}
                                        ret={r}
                                        onEdit={e => { e.stopPropagation(); setModal({ mode: "update", id: r._id }); }}
                                        onDelete={e => handleDelete(r._id, e)}
                                    />
                                ))}
                            </tbody>
                        </table>
...
function ReturnRow({ ret, onEdit, onDelete }) {
    const date   = new Date(ret?.returnDate ?? ret?.createdAt).toLocaleDateString();
    ...
    return (
        <tr className="transition" style={{ borderBottom: "1px solid var(--border)" }}
            ...
            <td className="px-4 py-3 text-center" style={{ color: "var(--ink)" }}>
                {ret?.totalItems ?? ret?.items?.length ?? 0}
            </td>
