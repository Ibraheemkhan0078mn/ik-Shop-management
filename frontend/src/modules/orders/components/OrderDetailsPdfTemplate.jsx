    ...
    return (
        <div style={styles.page}>
            ...
            <table style={styles.table}><thead><tr style={{ backgroundColor: "#111827", color: "#ffffff" }}>
                <th style={styles.headerCell}>#</th><th style={styles.headerCell}>Product Name</th><th style={styles.headerCell}>Batch No</th><th style={styles.rightHeaderCell}>Unit Price</th><th style={styles.rightHeaderCell}>Item Discount</th><th style={styles.rightHeaderCell}>Item Tax</th><th style={styles.rightHeaderCell}>Overall Discount Share</th><th style={styles.rightHeaderCell}>Sold Value Per Unit</th><th style={styles.rightHeaderCell}>Quantity</th><th style={styles.rightHeaderCell}>Subtotal</th>
            </tr></thead><tbody>
                {items.map((item, index) => {
                    ...
                    return <tr key={index} style={{ borderBottom: "1px solid #e5e7eb" }}>
                        ...
                        <td style={styles.cell}>{item.name || "-"}{item.portionType && <span style={{ fontSize: "0.75rem", color: "#6b7280" }}> ({item.portionType})</span>}</td>
            ...
            <div style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
                <div style={{ border: "1px solid #d1d5db", borderRadius: "0.5rem", overflow: "hidden" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.5rem", backgroundColor: "#111827" }}>
                        <span style={{ fontSize: "0.875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "#ffffff" }}>Grand Total</span>
                        <span style={{ fontSize: "1.5rem", fontWeight: 800, color: "#ffffff" }}>Rs {formatNumber(order?.totalAmount)}</span>
                    </div>
                </div>
            </div>
