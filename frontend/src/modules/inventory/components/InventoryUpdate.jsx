    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            if (result?.success) {
                ...
                setVisibility(false);
            } else toast.error(result?.reason || "Update failed");
        } catch (error) {
            console.error(error);
            ...
            <div className="bg-[var(--surface)] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto mx-4 custom-scrollbar">
                ...
                <div className="flex items-center justify-between px-8 py-5 border-b border-[var(--border)] sticky top-0 bg-[var(--surface)] z-10 rounded-t-2xl">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-bold text-[var(--ink)] font-display">Update Inventory Item</h2>
                            <p className="text-xs text-[var(--muted)] font-mono">{itemData?._id}</p>
                        </div>
                    </div>
                ...
                <div className="px-8 py-6 grid grid-cols-2 gap-5">
                    <div className="col-span-2">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Field label="Total Qty">
                                <input type="number" onWheel={e => e.target.blur()} name="totalQuantity" value={form.totalQuantity} onChange={handleChange} className={inputCls} min="0" />
                            </Field>
                        </div>
                    </div>
                    ...
                    <div className="col-span-2">
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <Field label="Vendor">
                                <input name="vendor" value={form.vendor} onChange={handleChange} className={inputCls} />
                            </Field>
                            <Field label="Purchase Price">
                                <input type="number" onWheel={e => e.target.blur()} name="purchasePrice" value={form.purchasePrice} onChange={handleChange} className={inputCls} />
                            </Field>
                            <Field label="Total Cost">
                                <input type="number" onWheel={e => e.target.blur()} name="totalCost" value={form.totalCost} onChange={handleChange} className={inputCls} />
                            </Field>
                            <Field label="Invoice Number">
                                <input name="invoiceNumber" value={form.invoiceNumber} onChange={handleChange} className={inputCls} />
                            </Field>
                ...
                <div className="flex items-center justify-end gap-3 px-8 py-5 border-t border-[var(--border)] sticky bottom-0 bg-[var(--surface)] rounded-b-2xl">
                    ...
                    <button onClick={handleUpdate} disabled={loading} className="px-6 py-2.5 rounded-xl bg-[var(--accent-2)] text-white font-semibold text-sm hover:opacity-90 transition-opacity disabled:opacity-60">
