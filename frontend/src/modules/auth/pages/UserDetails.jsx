...
import { ArrowLeft, User as UserIcon, Mail, Shield, Calendar, Lock } from "lucide-react";
...
export default function UserDetails() {
    ...
    const { data: responseWithPassword, isLoading: isLoadingWithPassword, error: errorWithPassword } = useGetUserByIdWithPasswordQuery({ id: userId, requesterRole: currentUserRole }, { skip: !isAdmin });
    const { data: responseWithoutPassword, isLoading: isLoadingWithoutPassword, error: errorWithoutPassword } = useGetUserByIdQuery(userId, { skip: isAdmin });
    const response = isAdmin ? responseWithPassword : responseWithoutPassword;
    const isLoading = isAdmin ? isLoadingWithPassword : isLoadingWithoutPassword;
    const error = isAdmin ? errorWithPassword : errorWithoutPassword;
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    <div className="rounded-2xl p-6 mb-6" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>
                        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
                            <div className="w-24 h-24 rounded-lg overflow-hidden shrink-0" style={{ background: "var(--surface-muted)", border: "2px solid var(--accent-2)" }}>
                                {user.photo ? (
                                    <img
                                        ...
                                        onError={(e) => {
                                            ...
                                            e.target.parentElement.innerHTML = `<div class="w-full h-full flex items-center justify-center"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: var(--accent-2)"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg></div>`;
                                        }}
                                    />
                                ) : (
