...
// const buildOrderItems = (cart) => cart.map(item => ({
//   product: item._id, name: item.name, quantity: item.qty,
//   unitPrice: item.unitPrice, lineTotal: item.unitPrice * item.qty,
//   originalPrice: item.originalPrice ?? item.unitPrice,
//   portionType: item.portionType || "full",
//   batchId: item.batchId ?? null, batchNumber: item.batchNumber ?? null,
// }));
...
//   const [portionIndex, setPortionIndex] = useState(null);
//   const [portionType, setPortionType] = useState("full");
//   const [portionCustomPrice, setPortionCustomPrice] = useState("");
...
//   // ── RTK Mutations (FIXED: using the correct mutation hooks) ──────────────
//   const [addOrder, { isLoading: isOrderLoading }] = useAddOrder();
//   const [createHold, { isLoading: isHoldCreating }] = useCreateHoldOrder();
//   const [updateHold, { isLoading: isHoldUpdating }] = useUpdateHoldOrder();
//   const [deleteHold, { isLoading: isHoldDeleting }] = useDeleteHoldOrder();
...
//     setCart(prev => {
//       const exists = prev.find(i => isSameLine(i, product._id, portionType, finalPrice, batchId));
//       if (exists) return prev.map(i => isSameLine(i, product._id, portionType, finalPrice, batchId) ? { ...i, qty: i.qty + 1 } : i);
//       return [...prev, {
//         ...product,
//         qty: 1,
//         unitPrice: finalPrice,
...
//       const duplicate = updated.findIndex(i =>
//         i._id === item._id && i.portionType === portionType && i.unitPrice === newPrice && i.batchId === item.batchId
//       );
...
//           onClose={() => { setShowPortionModal(false); setPortionItem(null); setPortionIndex(null); setPortionCustomPrice(""); }}
//           onConfirm={handlePortionConfirm}
//           language={language}
//         />
//       )}
...
//       {showHeldOrders && (
//         <div className="fixed inset-0 z-40 bg-gray-900/20 backdrop-blur-sm" onClick={() => setShowHeldOrders(false)} />
//       )}
//     </div>
//   );
// }
...
//     >
//       {imageUrl ? (
//         <img src={imageUrl} alt={product.name} className="w-full aspect-square object-cover rounded-lg bg-gray-100"
//           onError={e => { e.target.style.display = "none"; }} />
//       ) : (
...
//         {product.name}
//       </p>
//       <p className="text-sm font-bold text-cyan-700">Rs {(Number(product.price) || 0).toLocaleString()}</p>
//       {product.category?.name && (
//         <span className="text-[10px] text-gray-400 truncate w-full text-center">{product.category.name}</span>
//       )}
//     </button>
//   );
// }
