...
const InvesterPaymentUpdate = ({ setVisible, teacherId, paymentData, getInvesterPayments }) => {
    const [formData, setFormData] = useState({
        salaryAmount: paymentData?.salaryAmount || "",
        paymentMethod: paymentData?.paymentMethod || "cash",
        date: paymentData?.date ? paymentData.date.slice(0, 10) : "",
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await api.put(`/memberRoutes/updateSalaryPayment`, {
                ...formData,
                salaryPaymentId: paymentData?._id,
            });
            if (res.data.success) {
                getInvesterPayments();
                setVisible(false);
                    ...
                    <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-3">
                            ...
                            <h1 className="text-3xl font-black text-slate-800 tracking-tighter">
                                Update <span className="text-cyan-500">Invester Payment</span>
                            </h1>
                        </div>
                        <p className="text-sm font-semibold text-slate-400 mt-2 ml-1 flex items-center gap-2">
                            ...
                            Editing existing financial disbursement
                        </p>
                    </div>
                </div>
...
export default InvesterPaymentUpdate;