...
const TeacherUpdate = ({ teacherId, setVisibility, setData }) => {
    ...
    let teacherUpdateMutation = useUpdateTeacher()
    ...
    const [formData, setFormData] = useState({
        ...
        address: '', post: 'investor', education: '',
        isSalary: "",
        salary: "",
    ...
    // --- DATA HYDRATION (Bina flow bigade) ---
    useEffect(() => {
        if (teacherData) {
            setFormData({
                ...formData,
                teacherDocId: teacherData._id,
                teacherId: teacherData.instituteId || '',
                name: teacherData.name || '',
                fatherName: teacherData.fatherName || '',
                phone: teacherData.phoneNo || '',
                email: teacherData.email || '',
                address: teacherData.address || '',
                post: teacherData.post || 'investor',
                education: teacherData.education || '',
                salary: teacherData.salary || '',
                isSalary: teacherData.salary && teacherData.salary > 0 || false,
                isAbsenceCutEnabled: teacherData.perAttendenceCut > 0,
                perAttendenceCut: teacherData.perAttendenceCut || 0,
                cnicNo: Number(teacherData.cnic) || '',
                hiringDate: teacherData.hiringDate ? new Date(teacherData.hiringDate).toISOString().split('T')[0] : '',
                isPartner: true,
                partnerType: teacherData.partnerType || '',
                overallPartnerShareValue: teacherData.overallPartnerShareValue || '',
                bankName: teacherData.bankName || '',
                accountNumber: teacherData.accountNumber || '',
                notes: teacherData.notes || '',
                givenClasses: teacherData.givenClasses || [],
                languages: teacherData.languages || [],
                skills: teacherData.skills || [],
                experiences: teacherData.experiences || [],
                educationDegrees: teacherData.educationDegrees || [],
                teacherProfileImage: null,
                isActive: teacherData.isActive,
            });

            if (teacherData.profileImage) {
                setImagePreview(`http:///uploads/${teacherData.profileImage}`);
            }
        }
    }, [teacherData]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.keys(formData).forEach(key => {
                if (!['teacherProfileImage', 'languages', 'skills', 'experiences', 'educationDegrees', 'givenClasses'].includes(key)) {
            ...
            // const response = await api.post(`/memberRoutes/teacherUpdate`, data, {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative max-h-[95vh] w-full max-w-[90%] bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row animate-in zoom-in-95">
                ...
                <div className="lg:w-[20%] bg-linear-to-b from-cyan-700 to-cyan-600 p-8 flex flex-col items-center border-r border-slate-100">
                    ...
                    <h2 className="text-xl font-black text-white text-center uppercase tracking-tighter">Modify<br />Faculty</h2>
                    <p className="text-[10px] text-cyan-200 font-bold mt-2 uppercase tracking-widest">{formData.post}</p>
                </div>
                        ...
                        <section className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div className="relative">
                                    ...
                                    <input name="teacherId" value={formData.teacherId} onChange={handleChange} readOnly={!isRollEditable} className={`w-full bg-slate-50 border-2 rounded-2xl px-4 py-3 font-bold ${isRollEditable ? 'border-cyan-500' : 'border-transparent text-slate-400'}`} />
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {allClassesData?.map((cls) => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-xl text-[12px] font-black uppercase transition-all border-2 ${formData.givenClasses.includes(cls._id) ? 'bg-slate-900 border-slate-900 text-white' : 'bg-white border-slate-100 text-slate-400'}`}
                                        >
                                            {cls.className}
                                        </button>
                                    ))}
    ...
    onWheel={(e) => e.target.blur()} value={formData.overallPartnerShareValue} onChange={handleChange} icon="ri-percent-line" />
                        ...
                        {showAdvancedFields && (
                            <div className="space-y-8 animate-in slide-in-from-top-4 duration-300">
                                ...
                                <div className="w-full">
                                    <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Update Notes</label>
                        ...
                        <button type="submit" className="w-full py-5 bg-cyan-600 hover:bg-slate-900 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3">
                            <i className="ri-save-3-fill text-xl"></i> Update Faculty Records
                        </button>
                    </form>
                </div>
            </div>
...
// --- Atomic Helper Components (Synced with Create) ---
