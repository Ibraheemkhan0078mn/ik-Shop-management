...
const MemberUpdate = ({ memberId, setVisibility, setData }) => {
    ...
    console.log("From invester update", memberData)
    let { data: allClassesData } = useGetAllClassesQuery()
    let [updateMember] = useUpdateMemberMutation()
    ...
    useEffect(() => {
        console.log("THe if before")
        if (memberData) {
            console.log("Member data is found in investor update")
            ...
            setFormData({
                ...
                post: memberData.post || 'investor',
                education: memberData.education || '',
                salary: memberData.salary || '',
                isSalary: memberData.salary && memberData.salary > 0 || false,
                isAbsenceCutEnabled: memberData.perAttendenceCut > 0,
                perAttendenceCut: memberData.perAttendenceCut || 0,
                cnicNo: Number(memberData.cnic) || '',
                hiringDate: memberData.hiringDate ? new Date(memberData.hiringDate).toISOString().split('T')[0] : '',
                isPartner: true,
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.keys(formData).forEach(key => {
                if (!['memberProfileImage', 'languages', 'skills', 'experiences', 'educationDegrees', 'givenClasses'].includes(key)) {
            ...
            // const response = await api.post(`/memberRoute/memberUpdate`, data, {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative max-h-[95vh] w-full max-w-[90%] bg-surface rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col lg:flex-row app-enter">
                ...
                <div className="lg:w-[20%] app-gradient-vertical p-8 flex flex-col items-center border-r border-edge">
                    ...
                    <h2 className="text-xl font-black text-primary-foreground text-center uppercase tracking-tighter">Modify<br />Faculty</h2>
                        ...
                        <section className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                <div className="relative">
                                    ...
                                    <input name="memberId" value={formData.memberId} onChange={handleChange} readOnly={!isRollEditable} className={`w-full bg-surface-muted border-2 rounded-2xl px-4 py-3 font-bold ${isRollEditable ? 'border-edge-brand' : 'border-transparent text-ink-subtle'}`} />
    ...
    onWheel={(e) => e.target.blur()} value={formData.overallPartnerShareValue} onChange={handleChange} icon="ri-percent-line" />
                        ...
                        {showAdvancedFields && (
                            <div className="space-y-8 app-enter">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    ...
                                    <CustomInput type={"text"} label="Highest Education" name="education" value={formData.education} onChange={handleChange} icon="ri-graduation-cap-line" />
                                    <CustomInput type={"text"} label="Bank Name" name="bankName" value={formData.bankName} onChange={handleChange} icon="ri-bank-line" />
                                ...
                                <div className="w-full">
                                    <label className="block text-[10px] font-black text-ink-subtle uppercase mb-2 ml-1">Update Notes</label>
                        ...
                        <button type="submit" className="w-full py-5 bg-primary hover:bg-inverse text-primary-foreground rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95 flex items-center justify-center gap-3">
