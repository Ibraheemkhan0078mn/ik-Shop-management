...
import { useDeleteMemberDocumentMutation, useGetMemberByIdQuery, useUploadMemberDocumentMutation } from '../api/member.rtk.api';
...
const MemberDocumentsComp = ({ memberId }) => {
    ...
    return (
        <section className="bg-surface p-8 rounded-[2.5rem] border border-edge/60 shadow-sm relative overflow-hidden">
            ...
            {memberDocuemntImages && memberDocuemntImages.length > 0 ? (
    ...
    <div key={index} className="group relative aspect-square rounded-3xl border border-edge bg-surface-muted shadow-sm cursor-pointer overflow-hidden">
        ...
        <PermissionGuard permission={"member-documents-delete"}>
            <button
                onClick={(e) => { e.stopPropagation(); handleDelete(imgName); }}
                className="absolute top-2 right-2 w-6 h-6 bg-surface rounded-full shadow-md text-ink-subtle hover:text-danger flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity z-10"
            >✕</button>
        </PermissionGuard>
    </div>
))}
                ...
                <div className="border-t border-edge pt-8 text-center text-ink-muted py-6 italic text-sm">
