...
const AdminSideMemberFilteration = ({ MemberFilterationObj, setMemberFilterationObj, setMemberFilterationPanelVisibility, memberFilterationPanelVisibility }) => {
    ...
    async function handleRemoveAllMemberFilterFunction() {
        setMemberFilterationObj({
            memberInstituteId: "",
            memberName: "",
            post: "",
            isActive: true
    ...
    return (
        ...
        <div className={`fixed top-0 right-0 h-screen w-full sm:w-[420px] z-[100] bg-surface border-l border-edge shadow-[-20px_0_50px_rgba(0,0,0,0.05)] transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${memberFilterationPanelVisibility ? "translate-x-0" : "translate-x-full"}`}>
            ...
            <div className="px-6 sm:px-8 py-6 border-b border-edge flex items-center justify-between bg-surface shrink-0">
                ...
                <button
                    onClick={() => setMemberFilterationPanelVisibility(false)}
                ...
                <div className="space-y-5">
                    <div className="flex flex-col gap-2 group">
                        ...
                        <input
                            ...
                            type="text"
                            value={MemberFilterationObj.memberInstituteId}
                            onChange={(e) => setMemberFilterationObj({ ...MemberFilterationObj, memberInstituteId: e.target.value })}
                    ...
                    <div className="flex flex-col gap-2 group">
                        ...
                        <input
                            className="w-full bg-surface-muted border-2 border-transparent rounded-2xl py-3 px-5 outline-none transition-all duration-300 font-bold text-ink focus:border-edge-brand/20 focus:bg-surface focus:ring-4 focus:ring-primary/5 placeholder:font-normal"
                            type="text"
                            value={MemberFilterationObj.memberName}
                            onChange={(e) => setMemberFilterationObj({ ...MemberFilterationObj, memberName: e.target.value })}
                ...
                <div className="flex flex-wrap gap-2">
                    {[
                        ...
                        <button
                            ...
                            onClick={() =>
                                setMemberFilterationObj({
                                    ...MemberFilterationObj,
                                    isActive: item.value,
                                })
                            }
                            className={`px-4 py-2 rounded-xl font-bold text-[11px] uppercase tracking-wider transition-all duration-300 border-2
        ${MemberFilterationObj.isActive === item.value
                                    ? "bg-success border-emerald-500 text-primary-foreground shadow-md shadow-emerald-100"
                                    : "bg-surface border-edge text-ink-subtle hover:border-success-muted hover:text-success"
                                }`}
                        >
                            {item.label}
                        </button>
